<#
.DESCRIPTION
    This script will use a CSV to add domains and emails to blocked senders and domains.
...
.EXAMPLE
    Blocking an Email Address list from csv file for a single user
    Add-TrustedSender.ps1 -WorkerEmail "user@company.com" -UserEmail "getuser@company.com" -ImportFile "C:\temp\tested.csv"

.EXAMPLE
    Blocking an Email Address list from csv file for all user mailboxes
...
Try {
  ...
  # MailboxJunkEmailConfiguration's BlockedSendersAndDomains, but in a ton of testing I just
  ...
  if ($UserEmail -eq "*") {
    Get-Mailbox -ResultSize unlimited -RecipientTypeDetails UserMailbox | Set-MailboxJunkEmailConfiguration -BlockedSendersAndDomains $AddressHash -ErrorAction SilentlyContinue
  } else {
    Set-MailboxJunkEmailConfiguration -Identity "$UserEmail" -BlockedSendersAndDomains $AddressHash
  }
